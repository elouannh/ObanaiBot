    ...
    async run() {
        ...
        if (!["meadow", "forest", "dark_forest"].includes(area.biome)) {
            ...
            for (const z of map.Regions.filter(r => r.accesses.includes(loc.id))) {
                const tempoString = [];
                for (const a of z.Areas.filter(at => ["meadow", "forest", "dark_forest"].includes(at.biome))) {
                    tempoString.push(`${a.name}`);
                }
        ...
        if (seeds < 100) {
            return await this.ctx.reply(
                "Oups...",
                `Vous n'avez pas assez de graines.\n**Graines requises: ${seeds}/100**`,
                null,
                null,
                "warning",
            );
        ...
        if (worms < 25) {
            return await this.ctx.reply(
                "Oups...",
                `Vous n'avez pas assez de vers de terre.\n**Vers requis: ${worms}/25**`,
                null,
                null,
                "warning",
            );
        ...
        await this.ctx.reply(
            "Appâtage d'oiseaux.",
            "Vous tentez d'appâter des oiseaux aux alentours... Voyons voir...",
            "🐦",
            null,
            "outline",
        );
        ...
        if (!birdFound) {
            const resp = [
                "Après quelques minutes, un oiseau pointa son bec. "
                +
                "Cependant, il avait l'air bien trop frêle pour servir de corbeau messager...",

                "Après quelques minutes, toujours aucun oiseau... L'appât n'a servi a rien.",

                "Après quelques minutes, des rongeurs commencent à venir grignoter votre appât. "
                +
                "Ce n'est pas vraiment l'animal que vous souhaitiez rencontrer...",
            ];
        ...
        const rarity = luck(y);
        const kasugais = fs.readdirSync("./src/elements/kasugai_crows")
                           .map(e => require(`../../elements/kasugai_crows/${e}`))
                           .filter(e => e.rarity === rarity);

        const kasugai = kasugais[Math.floor(Math.random() * kasugais.length)];

        const [actualCrow, actualCrowLevel] = [
            iDatas.kasugai_crow === null ? null : require(`../../elements/kasugai_crows/${iDatas.kasugai_crow}`),
            calcCrowLevel(iDatas.kasugai_crow_exp),
        ];
        const supStr = `${
            actualCrow === null ? "Voulez-vous le récupérer et en faire votre oiseau ?"
            : `Vous avez déjà **${actualCrow.name}** (Rareté: ${"💎".repeat(actualCrow.rarity)}`
              +
              `${"⚫".repeat(5 - actualCrow.rarity)}), niveau **${actualCrowLevel.level} `
              +
              `(${actualCrowLevel.exp} exp)**, voulez-vous le remplacer ? `
              +
              "Toute progression en niveaux de corbeau sera perdue."
        }`;
        ...
        if (this.ctx.isResp(choice, "y")) {
            ...
            return await this.ctx.reply(
                "Appâtage d'oiseaux.",
                `Vous avez donc apprivoisé **${kasugai.name}** !`,
                "🐦",
                null,
                "outline",
            );
            ...
            return await this.ctx.reply(
                "Appeâtage d'oiseaux.",
                "Vous avez décidé de ne pas récupérer l'oiseau.",
                "🐦",
                null,
                "outline",
            );
