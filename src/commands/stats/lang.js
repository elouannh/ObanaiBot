...
class Lang extends Command {
    ...
    async run() {
        ...
        if (!this.args[0] || !allowedLangs.includes(this.args[0]?.toLowerCase() ?? "")) return this.ctx.reply("Votre langue n'existe pas.", "Si vous cherchez une langue, utilisez une ci-dessous :\n\nt:flag_fr: | `fr` Je parlerai français !\nt:flag_gb: | `en` I will speak english !\n", null, null, "error");
