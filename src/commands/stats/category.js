...
class Category extends Command {
    ...
    async run() {
        ...
        let categories = fs.readdirSync("./src/elements/categories")
                           .map(e => require(`../../elements/categories/${e}`));
        ...
        const msg = await this.ctx.reply(
            ...
            +
            "Si vous désirez améliorer votre catégorie (pour augmenter les bonus), "
            +
            "écrivez votre catégorie actuel, ainsi que votre souffle actuel **si demandé.**\n"
            +
            "Si vous désirez changer de catégorie, écrivez une catégorie différente de la votre. "
            +
            "Si vous désirez changer changer de souffle (si il y en a plusieurs dans votre catégorie), "
            +
            "écrivez votre catégorie actuelle et ensuite un autre souffle.\n"
            +
            "\n**En cas de changement soit de souffle soit de catégorie, "
            +
            "toute progression en amélioration sur votre catégorie sera définitivement perdue.**"
            +
        ...
        if (categories.map(e => e.label).includes(choice)) {
            ...
            if (breaths.length > 1) {
                const msg2 = await this.ctx.reply(
                    ...
                    +
                    `${
                        breaths.map(e => require(`../../elements/breaths/${e}_style.json`))
                               .map(e => `${e.emoji} \`id:${e.id}\` | ${e.name}`)
                               .join("\n")
                    }`
                    +
                    "\n\nRépondez ci-dessous avec l'id. Répondez avec `n` (non) pour annuler.",
            ...
            const masteries = "grimoires" in iDatas ?
                              ("mastery" in iDatas.grimoires ? iDatas.grimoires["mastery"] : 0)
                              : 0;

            if (masteries <= 0) {
                return await this.ctx.reply(
                    "Oups...",
                    "Vous n'avez pas de **Grimoire de maîtrise** en stock.",
                    null,
                    null,
                    "warning",
                );
            ...
            if (cat.label === pDatas.category && breath === pDatas.breath) {
                if (pDatas.categoryLevel === 5) {
                    return await this.ctx.reply(
                        "Oups...",
                        "Votre catégorie est déjà au niveau maximum.",
                        null,
                        null,
                        "warning",
                    );
            ...
            return await this.ctx.reply(
                "Changement/amélioration de catégorie.",
                "Vous avez décidé de ne pas changer de catégorie, ou de l'améliorer.",
                "👑",
                null,
                "outline",
            );
