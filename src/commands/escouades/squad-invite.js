...
class SquadInvite extends Command {
    ...
    async run() {
        ...
        if (!ready) return await this.ctx.reply("Oups...", "Le joueur ciblé est déjà occupé avec une autre commande.", null, null, "warning");
        ...
        if (this.ctx.isResp(choice, "y")) {
            ...
            return await this.ctx.reply("Inviter un nouveau membre.", "Le joueur a refusé de rejoindre l'escouade.", "⛩️", null, "outline");
        }
        else {
            return await this.ctx.reply("Inviter un nouveau membre.", "La commande n'a pas aboutie.", null, null, "timeout");
        }
    }
}
